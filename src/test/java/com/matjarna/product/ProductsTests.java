...
public class ProductsTests {
	...
	public void testGetProductById(Long productId, ProductDto productDto, boolean expectSuccess) throws Exception {
		MvcResult result = mockMvc
				.perform(get("/api/private/product/{id}", productId).header("Authorization", "Bearer " + token))
				.andReturn();
		int status = result.getResponse().getStatus();
		if (expectSuccess) {
			...
			assertEquals(String.format("Test failed on product id = %d ", productId), 400, status);
		}
	}
	...
	static Stream<Arguments> publicProductIdProvider() {
		return Stream.of(
				// Valid product IDs
				Arguments.of(2L, products.get(1), "us", "en", true),
				Arguments.of(2L, products.get(1), "ps", "ar", true),
				Arguments.of(2L, products.get(1), "us", "ar", true),
				Arguments.of(2L, products.get(1), "us", "en", true),
				Arguments.of(4L, products.get(3), null, null, true),

				// Non-existing product ID
				Arguments.of(999L, null, null, null, false), Arguments.of(-5L, null, null, null, false));
	}

	@Order(27)
	@ParameterizedTest
	@MethodSource("publicProductIdProvider")
	public void testPublicGetProductById(Long productId, ProductDto productDto, String country, String language,
			boolean expectSuccess) throws Exception {

		MockHttpServletRequestBuilder requestBuilder = get("/api/product/{id}", productId);
		if (country != null) {
			requestBuilder.param("country", country);
		} else {
			country = "us";
		...
		if (expectSuccess) {
			...
			assertEquals("Returned product code does not match the actual product code",
					productDto.getCode().toLowerCase(), responseDto.getCode().toLowerCase());
			assertEquals(
					String.format("Test failed on product id = %d because one or more filters were not met", productId),
					true, ProductHelperMethods.filtersChecker(responseDto, true, null, language, country, null, null));
		} else {
			assertEquals(String.format("Test failed on product id = %d ", productId), 400, status);
		}
	}
