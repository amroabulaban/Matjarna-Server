...
public interface IProductRepository extends JpaRepository<Product, Long> {
	...
	@Query("select distinct p from Product p left join fetch p.descriptions d left join fetch d.language l "
			+ "left join fetch p.prices pr left join fetch pr.country c "
			+ "where (:language is null or l = :language) and (:country is null or c = :country) and p.id = :id")
	Product findProductById(Long id, Country country, Language language);
